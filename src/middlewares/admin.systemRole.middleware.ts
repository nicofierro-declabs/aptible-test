...
export const adminSystemRoleMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.claims?.userId;
    if(userId) {
        const user = await portalUserService.getUserById(userId);
        if(user.systemRoleId && user.systemRoleId !== 1) apiResponses.unauthorized(res, 'Access denied');
        else next();
    }
}